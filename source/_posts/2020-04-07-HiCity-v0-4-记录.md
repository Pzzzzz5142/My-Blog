...
这一段代码中，`hbox	`表示我们创建了一个水平的box。他的`addStretch`方法则表示增加一个拉伸因子。`addWidget	`表示我们增加一个组件进去。这个拉伸因子是干嘛的呢？他是用来控制组件之间的间隙比例的。按照顺序加入则表示从左到右的元素是什么，而这些元素之间的间距比例又是多少。![截屏2020-04-07 下午6.57.23](截屏2020-04-12-下午3.29.51 "把我们的主页面拉大一点")
